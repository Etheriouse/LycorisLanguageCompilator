...
class BuilderAsd : public lycorisBaseVisitor
{

public:
    Program *build(const char *filename)
    {
        std::ifstream stream;
    ...
    std::any visitInstruction(lycorisParser::InstructionContext *ctx) override
    {
        ...
        else if (ctx->forblock)
        {
            ...
            Affectation *aff = nullptr;
            if (ctx->dinit)
            {
                if (ctx->tinit->children.size() > 1 && ctx->dinit->sarr->INT().size() > 0)
                    throw runtime_error("can't create a pointer static array");

                Type t = any_cast<Type>(ctx->tinit->accept(this));

                if (ctx->tinit->children.size() > 1)
                {
                    for (auto child : ctx->tinit->children)
                    {
                        if (child->getText() == "*")
                        {
                            t = Type::Pointer(t);
                ...
                for (auto size : ctx->dinit->sarr->INT())
                {
                    t = Type::Array(t, stoi(size->getText()));
                }
                vector<Declaration::Declare> vde;
                Expression *e = any_cast<Expression*>(ctx->dinit->value->accept(this));
                vde.push_back({
                    ctx->dinit->IDENT()->getText(),
                    Optional<Expression>::of(e)
                });
                d = new Declaration(t, vde);
            }

            if(ctx->forcond) {
                cond = any_cast<Expression*>(ctx->forcond->accept(this));
            }

            if(ctx->aftername) {
                aff = new Affectation(ctx->aftername->getText(), any_cast<Expression*>(ctx->after->accept(this)));
            }

            Instruction* block = any_cast<Instruction*>(ctx->forblock->accept(this));
            return (Instruction *)new For(d, cond, aff, block);
        }
        else if (ctx->ret)
        {
